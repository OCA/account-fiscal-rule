...
class TestInvoiceEcotaxCommon(BaseCommon):
    ...
    def setUpClass(cls):
        ...
        # Odoo 19: account.tax requires company to have a country (country_id NOT NULL)
        cls.env.company.country_id = cls.env.ref("base.fr")

        # ACCOUNTING STUFF
        # Odoo 19: no chart of accounts is installed when using BaseCommon, so we
        # must create the minimum set of accounts and journals needed to post invoices.
        # 1- Receivable account (needed by partner onchange on account.move)
        ...
        # 3- Income account (needed for sale journal default account)
        ...
        # 4- Sale journal (required in Odoo 19 when no chart of accounts is installed)
        ...
        # 6- Tax group (required in Odoo 19)
        cls.invoice_tax_group = cls.env["account.tax.group"].create(
            {
                "name": "Tax Group 10%",
                "company_id": cls.env.user.company_id.id,
            }
        )
        # 7- Invoice tax with included price to avoid unwanted amounts in tests
        cls.invoice_tax = cls.env["account.tax"].create(
            {
                ...
                "tax_group_id": cls.invoice_tax_group.id,
        ...
        # MISC
        # 1- Invoice partner (with receivable/payable accounts for Odoo 19)
        cls.invoice_partner = cls.env["res.partner"].create({"name": "Test"})
        cls.invoice_partner.property_account_receivable_id = cls.receivable_account
        cls.invoice_partner.property_account_payable_id = cls.payable_account
