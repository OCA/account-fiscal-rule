...
class L10nEuOssWizard(models.TransientModel):
    ...
    def _get_default_company_id(self):
        return self.env['res.company']._company_default_get()[0]
    ...
    def _prepare_fiscal_position_vals(self, country, taxes_data):
        ...
        fiscal_pos_name += " (EU-TAX-%s)" % country.code
        return {
    ...
    def generate_eu_service(self):
        ...
        for country in self.todo_country_ids:
            ...
            fpos_id = self.env["account.fiscal.position"].search(
                [
                    ("country_id", "=", country.id),
                    ("vat_required", "=", False),
                    ("auto_apply", "=", True),
            ...
            if not fpos_id:
                ...
                self.update_fpos(fpos_id, taxes_data)
